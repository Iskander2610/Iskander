...
export function playMusicTrack() {
  ...
  const beat = 0.36;
  const notes = [392, 494, 587, 659, 587, 494, 440, 392];
  const bass = [98, 98, 130.81, 130.81, 146.83, 146.83, 130.81, 98];

  for (let index = 0; index < 16; index += 1) {
    const time = start + index * beat;
    playKick(audio, time);
    if (index % 2 === 1) playSnare(audio, time);
    playHat(audio, time + beat * 0.5);
    playTone(audio, bass[index % bass.length], time, beat * 0.78, 'sawtooth', 0.12);
    playTone(audio, notes[index % notes.length], time + beat * 0.08, beat * 0.45, 'square', 0.07);
  }

  playTone(audio, 784, start + 16 * beat, beat * 1.2, 'triangle', 0.08);
  return 16 * beat + 0.7;
}
